  ...
  try {
    ...
    const res = await fetch('/api/stripe/create', {
      ...
      body: JSON.stringify({
        ...
        customerEmail: session.user?.email ?? undefined, // optional but helpful
        currency: 'CAD'
      }),
    });
      ...
      <div className="grid md:grid-cols-3 gap-8 mb-12">
        {PACKAGES.map((p) => (
          <div
            ...
            className={`relative flex flex-col rounded-3xl p-8 shadow-lg border-2 transition-all duration-300 cursor-pointer h-full ${
              selected.id === p.id
                ? 'border-indigo-500 bg-white shadow-indigo-200 scale-[1.02]'
                : 'border-indigo-100 bg-white/90 hover:shadow-indigo-100'
            }`}
          ...
          >
            ...
            >
              {selected.id === p.id ? 'Selected' : 'Select Plan'}
            </button>
          </div>
        ))}
      </div>
              ...
              {!session ? (
                <div className="text-center py-8">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 bg-white border border-slate-200 hover:border-indigo-300 text-slate-700 font-medium py-3 px-6 rounded-xl transition duration-200 shadow-sm hover:shadow-md"
                  >
                    Sign in with Google
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                  <div>
                    <button
                      onClick={handlePay}
                      disabled={loading}
                      className="w-full py-3 rounded-xl font-medium bg-indigo-600 hover:bg-indigo-700 text-white transition-colors disabled:opacity-50"
                    >
                      {loading ? 'Redirecting…' : 'Go to Payment Page'}
                    </button>
                  </div>
